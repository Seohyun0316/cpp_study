...
int main() {
    ...
    if (!fout.is_open()) {
		cout << "파일 열기 실패!" << endl;// 파일 열기 실패 시 메시지 출력
        return 1;
    }
    ...
    while (true) {
        ...
        fout.write(reinterpret_cast<const char*>(&num), sizeof(num));  // 바이너리 파일에 정수 저장
    }
    fout.close();  // 파일 닫기
    ...
    while (fin.read(reinterpret_cast<char*>(&num), sizeof(num))) {
        ...
        fout2.write(reinterpret_cast<const char*>(&doubled_num), sizeof(doubled_num));  // 새로운 파일에 2배 값 저장
    }
